...
const Counter: React.FC<ICounter> = ({id, style, customStyle}) => {
    ...
    const quantity = cartState.cardItems.find((x: ICart) => x.item.id === id)?.quantity;

    return (
        <div className={`flex flex-row justify-center w-[90%] mx-4 lg:mx-2 h-fit items-center ${customStyle}`} style={style}>
